...
- **Dependencies**: None
- **Status**: **In progress.** Done so far: bottom nav (5 tabs), Dashboard's 2 clickable match cards, Chat's input/send/upload/message-list, `PredictScreen` + its 2 sub-tabs, `UpcomingScreen` + `LiveScreen` + Matchs' 2 sub-tabs, `PlayersScreen` (`players_search_a`/`players_search_b` inputs, `<tag>_result_<name>` dropdown rows, `players_follow_button`). Remaining: MatchDetail, Value's 4 sub-tabs (Value/Scanner/Stats/Edge).
...
- **Files involved**: `MainActivity.kt`, `DashboardScreen.kt`, `ui/components/ValueCard.kt`, `ChatScreen.kt`, `PredictScreen.kt`, `UpcomingScreen.kt`, `LiveScreen.kt`, `NavGroups.kt`, `PlayersScreen.kt`
