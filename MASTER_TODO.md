...
- **Dependencies**: None
- **Status**: **In progress.** Done so far: bottom nav (5 tabs), Dashboard's 2 clickable match cards, Chat's input/send/upload/message-list, `PredictScreen` + its 2 sub-tabs, `UpcomingScreen` + `LiveScreen` + Matchs' 2 sub-tabs, `PlayersScreen`, `MatchDetailScreen`, Value's 4 top-level sub-tabs (`value_tab_value`/`scanner`/`stats`/`edge` in `NavGroups.kt`) + `ValueScreen` itself (`value_refresh`, its own `value_subtab_today`/`history` inner tabs, `value_chip_confidence`, `value_load_history`). Remaining: `ScannerScreen`, `PerformanceScreen` (Stats), `EdgeScreen`.
