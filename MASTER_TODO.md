...
- **Dependencies**: None
- **Status**: **In progress.** Done so far: bottom nav (5 tabs), Dashboard's 2 clickable match cards, Chat's input/send/upload/message-list, `PredictScreen` + its 2 sub-tabs, `UpcomingScreen` + `LiveScreen` + Matchs' 2 sub-tabs, `PlayersScreen`, `MatchDetailScreen` (`matchdetail_back`, `matchdetail_screen` root — this screen has almost no other interactive elements to tag, it's mostly a read-only analysis view). Remaining: Value's 4 sub-tabs (Value/Scanner/Stats/Edge).
