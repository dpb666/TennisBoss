...
- **Dependencies**: None
- **Status**: **In progress.** Done so far: bottom nav (5 tabs), Dashboard's 2 clickable match cards, Chat's input/send/upload/message-list, `PredictScreen` + its 2 sub-tabs, `UpcomingScreen` + `LiveScreen` + Matchs' 2 sub-tabs, `PlayersScreen`, `MatchDetailScreen`, Value's 4 top-level sub-tabs + `ValueScreen`, `ScannerScreen`, `PerformanceScreen` (`performance_subtab_stats`/`calendar`, `performance_date_<date>` on each calendar date chip). Remaining: `EdgeScreen`.
