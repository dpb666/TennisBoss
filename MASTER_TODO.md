...
- **Dependencies**: None
- **Status**: **In progress.** Done so far: bottom nav (5 tabs), Dashboard's 2 clickable match cards, Chat's input/send/upload/message-list, `PredictScreen` + its 2 sub-tabs, `UpcomingScreen` (match cards `upcoming_card_*`, `upcoming_chip_odds`/`upcoming_chip_confidence` filter chips) + `LiveScreen` (`live_card_${event_id}`) + Matchs' 2 sub-tabs (`matches_tab_upcoming`/`matches_tab_live`). Remaining: Players, MatchDetail, Value's 4 sub-tabs (Value/Scanner/Stats/Edge).
...
- **Files involved**: `MainActivity.kt`, `DashboardScreen.kt`, `ui/components/ValueCard.kt`, `ChatScreen.kt`, `PredictScreen.kt`, `UpcomingScreen.kt`, `LiveScreen.kt`, `NavGroups.kt`
