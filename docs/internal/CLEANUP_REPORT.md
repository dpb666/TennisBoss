...
- **New, surfaced by the Compose BOM bump (`MASTER_TODO.md` #8)**: `TabRow` (Material3) is now deprecated in favor of `PrimaryTabRow`/`SecondaryTabRow`, used in `NavGroups.kt` (x3: Predict/Matchs/Value group tabs), `PerformanceScreen.kt`, `ValueScreen.kt`. **Not auto-fixed** — `PrimaryTabRow` changes the default visual style (different indicator/spacing), so swapping it is a UI decision, not a mechanical rename. Compiler warnings only, not errors.
