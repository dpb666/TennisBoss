...
- **Hardening** (commit `e2d2677`, deployed): user explicitly asked to "renforce contre ce bug" (tests + safeguards) rather than move on. Added 12 tests total — `tests/test_espn_api.py` (6, `espn_api.py` had zero test coverage before this), `tests/test_live_api_upcoming.py` (5, including "a live match that started yesterday must NOT be filtered out"), and one integration test in `tests/test_api_endpoints2.py`. Also added a **global safety-net filter** in `bot/api.py::api_upcoming()` itself, applied after all 4 sources are merged: any non-live fixture dated before today is dropped regardless of which source produced it, with a `WARN` log noting the count. Each individual source is already fixed, but this is a second line of defense — if a future 3rd data source (or a regression in an existing one) reintroduces the same bug class, it can no longer reach the app, only produce a log line. `python3 -m pytest` 369/369 passed (357 baseline + 12 new), deployed and confirmed healthy.
...
### Commits + activation prod préparée (2026-07-16, fin session CTO)
...
### D-4 : mécanisme de migrations versionnées livré (2026-07-23)

Plutôt que de continuer D-1 mécaniquement (voir juste au-dessus — le reste est plus risqué), pris le prochain item structurel de mon lane qui ne dépend d'aucune décision humaine : D-4, l'absence de mécanisme de migration versionné.

**Scope délibérément additif, rien touché à l'existant** : nouveau `bot/db_migrations.py` (table `schema_migrations`, runner `apply_pending_migrations()`) + nouveau dossier `data/db/migrations/` (vide aujourd'hui, avec un README expliquant la convention). Câblé dans `db.init()` juste après la liste `ALTER` existante — la liste de ~34 entrées ad-hoc reste intacte et continue de marcher exactement comme avant, réécrire cet historique contre le schéma prod réel n'en valait pas le risque pour zéro changement de comportement. Convention : fichiers `.sql` numérotés (`NNNN_description.sql`), appliqués dans l'ordre, chacun commit séparément — un échec sur le fichier N n'affecte pas 1..N-1 déjà appliqués et bloque juste N+1... sans les tenter.

**Vérifié, pas juste testé en synthétique** : 9 nouveaux tests (`tests/test_db_migrations.py` — table créée même sans fichier, application/ordre, idempotence, échec isolé, intégration avec `db.init()`), 725/725 en local (716 + 9). En plus des tests unitaires : copié le vrai `state/tennisboss.db` du poste de dev (schéma réel, pas une DB synthétique) vers un scratch, tourné `db.init()` dessus avec le nouveau mécanisme actif — résultat : 21 tables avant → 22 après, la seule table ajoutée est `schema_migrations` elle-même, aucune dérive de schéma, aucune perte de données. C'est le genre de vérification qui aurait attrapé une régression que les tests synthétiques auraient pu manquer (leçon du bug de clé calibration_bins/bins plus tôt cette session).

**Pourquoi le registre reste 🟢 et pas fermé complètement** : le mécanisme existe et est prouvé sûr, mais aucune vraie migration n'a encore été écrite (rien n'en a besoin aujourd'hui) — la preuve complète du chemin de bout en bout viendra du prochain vrai changement de schéma, qui devra passer par un fichier `data/db/migrations/000X_....sql` plutôt qu'une 35e entrée ajoutée à la liste `ALTER`.

**Frozen core :** untouché (`git diff -- bot/predictor.py bot/calibrate.py` vide — seuls `bot/config.py`, `bot/db.py` modifiés, `bot/db_migrations.py`/`data/db/migrations/`/`tests/test_db_migrations.py` nouveaux).
