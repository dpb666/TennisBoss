...
| D-5 | Knowledge sprawl: ~25 overlapping audit MDs, stale ones misleading (AI_CHAT_AUDIT, REALTIME-ROI, PROJECT_STATUS lag) — **PROJECT_STATUS refreshed 2026-07-22, no longer stale**; `AI_CHAT_AUDIT.md`/`RELEASE_AUDIT.md` still at root, not yet moved to `docs/audits/` (cross-referenced from 5+ files each — archival needs a link-fix pass, not done this cycle) | 🔴 | root + `docs/` | Knowledge base with supersession (§6.4); audits → immutable `docs/audits/`; this blueprint as index |
