...
|----|------|-----|-------|------------------|
| D-1 | `bot/api.py` god-module — **progress confirmed 2026-07-23**: down to 2,724 lines (was ~4.2k), 28 routes still inline (was 52 — 29 already moved to `bot/blueprints/{core,matches,performance,personalization,diagnostics,reporting}.py`; diagnostics + reporting added this cycle, phases 2e/2f), the 4 daemon loops are now thin compat shims (2-5 lines each) delegating to `bot/workers/*.py` — see D-3. Still 🔴: far from the Q4 #7 exit criterion (`bot/api.py` < 500 lines, shim + factory only) | 🔴 | `bot/api.py` | §5.1/§5.3 blueprint split (Q4 #7) — remaining route batches |
