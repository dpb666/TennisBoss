...
|----|------|-----|-------|------------------|
| D-1 | `bot/api.py` god-module — **progress confirmed 2026-07-23**: down to 2,689 lines (was ~4.2k), 25 routes still inline (was 52 — 32 already moved to `bot/blueprints/{core,matches,performance,personalization,diagnostics,reporting}.py`; diagnostics/reporting/3 system routes added this cycle, phases 2e/2f/2g), the 4 daemon loops are now thin compat shims (2-5 lines each) delegating to `bot/workers/*.py` — see D-3. Still 🔴: far from the Q4 #7 exit criterion (`bot/api.py` < 500 lines, shim + factory only) — remaining 25 routes are mostly the core prediction/value/inplay business logic (`/api/predict`, `/api/insight`, `/api/value*`, `/api/inplay/*`) and admin-mutating ops (`/api/backfill`, `/api/settlement/run`, `/api/learn/run`, `/api/ingest/*`), meaningfully higher-risk to extract than the read-only batches done so far — deserves its own dedicated pass, not a mechanical continuation | 🔴 | `bot/api.py` | §5.1/§5.3 blueprint split (Q4 #7) — remaining route batches need more care |
...
| D-4 | ~~No schema migration mechanism~~ **mechanism shipped 2026-07-23** (`bot/db_migrations.py` + `data/db/migrations/` + `schema_migrations`, exactly per §7.2). Additive alongside — not replacing — the existing `CREATE TABLE IF NOT EXISTS` + ~34-entry ad-hoc `ALTER` list in `db.py::init()`, deliberately left as-is (rewriting working history against the live prod schema wasn't worth the risk for zero behavior change). Wired into `db.init()` automatically. Verified against a real copy of an existing populated DB, not just a fresh one: only `schema_migrations` itself got added, zero data/schema drift. Still 🟢 not fully 🟢-closed: zero real migrations have been written yet (none needed since no schema change is pending) — the register entry stays open until the *first* real migration proves the path end-to-end | 🟢 | `bot/db_migrations.py`, `data/db/migrations/` | Write the next schema change as a migration file (proves the mechanism for real) |
