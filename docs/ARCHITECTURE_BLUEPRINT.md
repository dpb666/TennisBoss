...
|----|------|-----|-------|------------------|
| D-1 | `bot/api.py` god-module (~4.2k lines: 52 routes + 4 daemon loops + caches + business logic) | 🔴 | `bot/api.py` | §5.1/§5.3 blueprint split + worker extraction (Roadmap Q3–Q4) |
| D-2 | No enforced module boundaries in flat `bot/` (69 files) | 🔴 | `bot/` | Context packages + import lint (ADR-004, §4) |
| D-3 | Daemon threads live inside the API process (scanner, followed-refresh, TG poll, settlement/CLV) | 🔴 | `bot/api.py` | Move to `ops/jobs/` registry (§5.3) — compose already models the correct split |
| D-4 | No schema migration mechanism (CREATE IF NOT EXISTS + ad-hoc ALTERs) | 🔴 | `bot/db.py` | `data/db/migrations/` + `schema_migrations` (§7.2) |
| D-5 | Knowledge sprawl: ~25 overlapping audit MDs, stale ones misleading (AI_CHAT_AUDIT, REALTIME-ROI, PROJECT_STATUS lag) | 🔴 | root + `docs/` | Knowledge base with supersession (§6.4); audits → immutable `docs/audits/`; this blueprint as index |
...
| D-7 | Hundreds of `tmp*.json.corrupt` files polluting `state/` (atomic-write leftovers) | 🟢 | `state/` | Janitor job + monitor count (§7.2); also indicates memory.save() failure path worth a look |
| D-8 | LAN unauthenticated-serve fallback when token unset | 🟡 | `bot/api.py:3182` | Default-deny in prod env (§11.1) |
| D-9 | `compare-engines` CLI documented but not registered in `run.py` | 🟢 | `run.py` | Wire it (1 slice); prerequisite for any future engine-v2 work |
| D-10 | Duplicated logic: two surface-detection paths (config keyword/city maps vs scanner's own ranking); Android `StatCard`/`RecentRow` byte-duplicates | 🟡 | `bot/config.py`/`api.py`; `EdgeScreen.kt`/`PerformanceScreen.kt` | One source of truth per fact (P10); extract shared composables |
...
| D-16 | `ksearch.py`, `watchdog.py`, `.openclaw*`, `MEMORY/SOUL/IDENTITY/TOOLS/HEARTBEAT.md` — openclaw-era artifacts of unclear liveness | 🟢 | root | Liveness audit; archive or document; root directory should teach the architecture, not its history |
| D-17 | Rate limiting absent on `/api/chat` (LLM cost) and on auth failures | 🟡 | `bot/api.py` | §11.4 |
| D-18 | Log file grows unbounded (no rotation found) | 🟢 | `bot/log.py` | Size-based rotation (§10) |
